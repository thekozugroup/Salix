...
def _title_for_feature(feature: str) -> str:
    title_overrides = {
        "ttr": "TTR convergence",
        "mtld": "MTLD convergence",
        "yule_k": "Yule's K convergence",
        "honore_r": "Honore's R convergence",
        "simpson_d": "Simpson's D convergence",
        "mean_word_len": "Mean word length convergence",
        "mean_sent_len": "Sentence length convergence",
        "stdev_sent_len": "Sentence length variation convergence",
        "comma_per_sentence": "Comma cadence convergence",
        "flesch_kincaid_grade": "Flesch-Kincaid grade convergence",
        "gunning_fog": "Gunning Fog convergence",
        "ari": "ARI convergence",
        "formality_f_score": "Formality F-score convergence",
    }
    ...
    payload = {
        ...
        "completion": {
            ...
            "overview_chart_count": len([
                chart for chart in charts if chart["feature"] in OVERVIEW_CHART_FEATURES
            ]),
        },
...
def render_svg(payload: dict, charts: list[dict] | None = None) -> str:
    ...
    height = 110 + panel_height * len(charts)
    parts = [
...
def write_chart_folder(payload: dict, charts_dir: Path) -> None:
    ...
    for old_svg in charts_dir.glob("*.svg"):
        old_svg.unlink()
    files = []
    ...
    (charts_dir / "README.md").write_text(render_charts_index(payload, files))
